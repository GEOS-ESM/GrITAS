...
def getconf(tresh,nob,chisql,chisqr,tstud):
  ...
  for k in range(nlev):
      ...
      id=where(nobs>tresh)
      nccum[k] = sum(nobs[id])
      print nccum[k], chisql[k,1,1], chisqr[k,1,1], ndim
...
#.................................
def show_plot_monthly_one0(lev,vals,anno,colors,case,scale,this):
...
#   ax.set_facecolor('#CECECE')
    ...
    ax.plot(omean,pos)
    ax.plot(100*ones(len(omean)), pos, color='k',linewidth=1, alpha=0.8)
...
#.................................
def show_plot_monthly_one(lev,vals,anno,colors,case,scale,this,confl,confr,studt):
...
g=GrITAS(fn,myrc,vname,vunits,confidence)
[nlev,nlat,nlon] = g.get_dims()
#[vunits,confidence] = g.get_info()
...
#   [vunits,confidence] = g.get_info()
    ...
    if confidence==True:
       ...
       [lon,lat,lev,mean,stdv,nob] = g.get_gritas_data()
    chnindx=lev;
       ...
       if nx==1:  # store result
          ...
          if valuesYaml['global']['compare monthly']['doit']==True:
             if   valuesYaml['global']['compare monthly']['type']=='ratio':
                ...
                stat=stats[0]
                if confidence==True:
                   cl=confl[:,ii,nr,0]*totals[:,ii,nr,0] # assumes nobs between exp is close
                   cr=confr[:,ii,nr,0]*totals[:,ii,nr,0] # assumes nobs between exp is close
                   show_plot_monthly_one(lev,totals[:,ii,nr,0],expsid,colors[0],'ratio',scale,yyyymm,cl,cr,studt)
                else:
                   show_plot_monthly_one0(lev,totals[:,ii,nr,0],expsid,colors[0],'ratio',scale,yyyymm)

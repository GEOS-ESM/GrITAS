#!/bin/csh
  set Usage   = "dsat2gritas2_ods.csh ExpID Instr    YYYYMM [-fetch -cleanup]"
  set Example = "dsat2gritas2_ods.csh merra amsua_am 200401 "
...
# ------------------------------
  ...
  set arch_anl_Files = ( `${name_cmd_basic} anl -ods` )
  if ( $status ) then
     $name_cmd_basic anl 
...
# --------------------------------------
  if ( $Fetch_ObFiles == $TRUE ) then
     dmget $arch_anl_Files &
     cp $arch_anl_Files . ;set Status = $status
     if ( $status ) then
...
# Set base options
# ----------------
  set RC_File          = ${RunDir}/rc_files/gritas_${Instr}_merra.rc
#  set Gritas_Core_Opt  = "-nlevs 50 -rc $RC_File -res d -ncf -ospl -lb -nopassive"
#  set Gritas_Core_Opt  = "-rc $RC_File -res d -ncf -ospl -lb -nopassive"
  set Gritas_Core_Opt  = "-rc $RC_File -res d -ospl -nopassive"
...
# ---------------------------------------------------
  ...
  foreach syn_time ( all 00 06 12 18 )
     if ( $syn_time == all ) then
        set anl_Files = ( `${name_cmd_basic} anl -t -ods`)
        set syn_tag   = ""

     else
        set anl_Files = ( `${name_cmd_basic} anl -t -ods -syn $syn_time`)
...
#    ... for o-f data
#    ----------------
     $gritas -omf $Gritas_Core_Opt ${anl_Files}; set Status = $status
     if ( $Status ) then
        echo "Error status (= ${Status}) returned from ${gritas}"
        goto cleanup
     endif
...
# -----------------------------
  set all_anl_Files = ( `${name_cmd_basic} anl -t -ods`)
